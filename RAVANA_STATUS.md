...
| Relational Inference Engine (2026-05-20) | **RIE v1**: ConceptEdge gets `relation_type` field. `infer_chain()`: sparse multi-hop with activation budget (frontier_budget=5), entropy penalty, coherence gate via core_vector, winner-take-most. `compress_paths()`: chains → shortcut edges. `find_analogy()`: structural pattern matching. **Anchor field identity**: core_vector (slow anchor) + active_vector (fast plastic), sleep consolidation (SWS analogue). Deep compositional (3 seeds): 3-hop chains **22% mean** (matches MLP), negative rejection **33% mean** (seed 456: 100%!), relational transfer 0%. Semantic fog controlled. |
