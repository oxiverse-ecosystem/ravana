...
| Semantic Geometry Dashboard (2026-05-21) | **Observability layer**: `graph_diagnostics()` computes 12+ metrics — graph entropy, activation spread, clustering coefficient, contradiction density, relation cluster separation (intra vs inter cosine sim), attractor stability (core vs genesis), core-active alignment, branching factor, edge weight stats, shortcut ratio, path degeneracy, inference sparsity (specificity, branching). `geometry_report()` for formatted output. `_inference_log` tracks last 50 inference runs. |
