...
| Relational Inference Engine (2026-05-20) | **RIE v1**: ConceptEdge gets `relation_type` field (semantic/causal/temporal/analogical/contextual/inferred). `infer_chain()`: multi-hop BFS with uncertainty accumulation, contradiction penalty, semantic alignment. `compress_paths()`: successful chains → shortcut edges. `find_analogy()`: structural pattern matching via relation-shape similarity. Forward pass integrates multi-hop inference. Sleep cycle compresses successful paths. Deep compositional: 3-hop chains 0% → **33%** (now matches MLP). Concept→token mapping identified as next bottleneck |
...
10. **~~No transitive inference~~** — **PARTIALLY RESOLVED** (2026-05-20) — RIE v1 added (infer_chain, compress_paths, find_analogy). Deep compositional: 3-hop chains 0% → 33% (matches MLP). Remaining bottleneck: concept→token mapping drifts during training, so inferred concept IDs don't cleanly map back to token IDs. Need stable concept→token binding or direct vector-space inference.
