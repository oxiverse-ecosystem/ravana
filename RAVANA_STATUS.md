...
| Relational Inference Engine (2026-05-20) | **RIE v1**: ConceptEdge gets `relation_type` field + `relation_vector` (8-dim learned embedding). `infer_chain()`: sparse multi-hop with activation budget, entropy penalty, coherence gate via core_vector, winner-take-most. `compress_paths()`: chains → shortcut edges. `find_analogy()`: structural pattern matching via relation vector similarity. **Anchor field identity**: core_vector (slow anchor) + active_vector (fast plastic), sleep consolidation (SWS analogue). **Relation embeddings**: learned from Hebbian correlation, enables analogical transfer ("planet orbits star" ≈ "electron orbits nucleus"). Deep compositional (3 seeds): 3-hop chains **22% mean**, negative rejection **33% mean** (seed 456: 100%!), relational transfer 0%. Semantic fog controlled. |
