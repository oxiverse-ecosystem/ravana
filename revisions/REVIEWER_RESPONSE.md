...
- [x] **Serialization fidelity:** Adjacency index desync + relation predictor serialization bugs traced and fixed — 67/67 tests pass (6 new stress tests), identity persistence 100% consistency, 5-cycle drift test, post-sleep roundtrip, cross-format consistency, large graph roundtrip
- [x] **Hybrid memory architecture:** SharedVectorIndex (cosine similarity retrieval), MemoryReconstructor (reconstructive recall from partial cues), natural Ebbinghaus decay per cycle, Hebbian model updates during sleep — 67/67 tests pass
