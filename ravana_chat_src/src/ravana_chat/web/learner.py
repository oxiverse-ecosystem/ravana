...
class WebLearner:
    ...
    def __init__(self, graph_engine, decoder_engine, glove_vector_fn,
                 ...
                 emotion_engine=None, identity_engine=None):
    ...
    def _decay_episodic_edges(self):
        ...
        Delegates to graph_engine which holds the episodic edges store.
        """
        ge = self.graph_engine
        if not hasattr(ge, '_episodic_edges'):
            return
        to_remove = []
        for (src, tgt), edge in ge._episodic_edges.items():
            edge.weight *= 0.90
            if edge.weight < 0.05:
                to_remove.append((src, tgt))
        for key in to_remove:
            del ge._episodic_edges[key]
            # Clean up src-indexed lookup
            src_id, tgt_id = key
            if src_id in ge._episodic_by_src:
                ge._episodic_by_src[src_id] = [(t, e) for t, e in ge._episodic_by_src[src_id] if t != tgt_id]
                if not ge._episodic_by_src[src_id]:
                    del ge._episodic_by_src[src_id]
