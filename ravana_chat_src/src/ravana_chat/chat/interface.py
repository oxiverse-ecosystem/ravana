...
class ChatInterface:
    ...
    def _init_components(self):
        ...
        # Cognitive core - identity & emotion first (needed by WebLearner)
        ...
        self._needs_seed_training: bool = True
        self._needs_synthetic_training: bool = True
    ...
    def _load_or_initialize(self):
        ...
        # Sync decoder vocab attributes
        self._decoder_vocab_built = self.decoder_engine._decoder_vocab_built
        self._decoder_word_to_embed = self.decoder_engine._decoder_word_to_embed
        self._decoder_word_to_idx = self.decoder_engine._decoder_word_to_idx
        
        # Train decoder on synthetic graph sentences for quality generation
        self.decoder_engine._train_decoder_from_graph(self.graph_engine, min_synthetic=2500)
        self._train_decoder_on_seed_corpus(max_sentences=250)
    ...
    def _train_decoder_on_seed_corpus(self, max_sentences: int = 250) -> int:
        ...
        if self.decoder_engine is None or not self._decoder_vocab_built:
            return 0
        import os
        ...
        with open(corpus_path, "r", encoding="utf-8") as f:
            text = f.read()
        sentences = [s.strip() for s in text.split(".") if len(s.strip()) > 5][:max_sentences]
        trained = 0
        for sent in sentences:
            words = sent.split()
            self.decoder_engine.train_on_sentence(words, self._decoder_word_to_embed, self._decoder_word_to_idx)
            trained += 1
        self.decoder_engine.sleep_cycle()
        self._decoder_training_count += trained
        print(f"  [Decoder] Seed corpus training: {trained} sentences")
        return trained
    ...
    def _generate_response(self, ctx: CognitiveResponseContext) -> Tuple[str, str]:
        ...
        _have_web = self.decoder_engine.web_training_count >= 50
        _total = self.decoder_engine.training_count
        if _have_web and _total >= 100:
            try:
    ...
    # Connector words for each relation type (for _walk_chain_simple)
    @property
    def _EDGE_CONNECTORS(self):
        return {
    ...
    # Connector set for _graph_fallback_response
    @property
    def _CONNECTOR_SET(self):
        connectors = set()
        for opts_list in self._EDGE_CONNECTORS.values():
            for tier_name, opts in opts_list:
                connectors.update(opts)
        return connectors
