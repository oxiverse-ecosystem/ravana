...
\author{[Author details removed for double-anonymized review]}
\date{}
...
Memory retrieval uses a shared vector index that computes cosine similarity between a query embedding and all stored memory embeddings. For collections under 10{,}000 vectors, the index uses a vectorized matrix multiply; for larger collections, it supports optional approximate nearest-neighbor indexing for $O(\log N)$ queries. This replaces earlier keyword-matching retrieval with a semantic similarity primary path, falling back to keyword search only when the vector index is empty.

\paragraph{Reconstructive recall.}
Human memory does not retrieve exact records; it reconstructs from fragments. RAVANA implements this via a reconstructive recall module that takes a partial cue, finds vector-similar candidates, spreads activation through the ConceptGraph to gather neighbor context, blends candidate content with neighbor information weighted by activation strength, and computes a fidelity score indicating how much of the result is direct recall versus inferred reconstruction.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{URL:} Available upon acceptance (repository link removed for double-anonymized review)
    \item \textbf{License:} Open-source license (details available upon acceptance)
