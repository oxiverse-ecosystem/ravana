...
Subsequent architectural improvements---subject-concept anchoring (activating the first token's concept at 80\% of max activation to prevent verb-domain bias), predicate matching on edges (boosting edges whose stored verb matches the input verb by 2.5$\times$ and penalizing mismatches by 0.4$\times$), non-subject target suppression (penalizing tokens from non-subject active concepts across all logit sources), and initializing concept vectors from token embeddings rather than random vectors (preventing concept conflation after interleaved replay)---substantially improved probe resolution. After these fixes, the \texttt{test\_full\_cross\_domain\_eval.py} suite reports 100\% top-1 on the 20-probe cross-domain set. The full experiment script (\texttt{experiment\_cross\_domain.py}) has not yet been re-run with these fixes; the results in Table~2 reflect the pre-fix configuration.

\begin{table}[h]
\centering
\caption{Cross-domain transfer probe performance.\footnote{Cross-domain probe results (15 probes, character-level tokenization, seed 42). Fair evaluation results from the separate fairness benchmark (\texttt{fair\_evaluation.json}). Forward transfer and zero-shot metrics from the cross-domain experiment.}}
\begin{tabular}{lcc}
\toprule
...
The limitations are substantial. The experiments are synthetic and internally reported. The architecture is small and not yet evaluated on large naturalistic cognitive benchmarks. The top-1 cross-domain transfer result in the reported experiment remains low at 14.3\%, though subsequent architectural fixes (subject-concept anchoring, predicate matching, concept vector initialization) have resolved probe-level failures; the full experiment script has not yet been re-run with these fixes. Local Hebbian and predictive-coding updates are less sample-efficient than backpropagation. Graph operations can become expensive at scale, especially if pairwise curvature or similarity computations are not sampled. Earlier code audits found bugs such as undertrained recurrent cells, unused normalization, lack of gating, and inefficient generation paths; the project reports state that these were fixed, but independent verification remains necessary. The current memory embedding uses a learned embedder (character n-gram with random projection, 64-dimensional), which provides better retrieval than hash-based approaches but remains a lightweight placeholder compared to full neural embeddings. Finally, the fairness benchmark is not a substitute for real-world ethical validation.

\section{Future Work}
Future work should prioritize independent replication, public benchmark definitions, and ablation-controlled evaluation. The most important technical directions are: (1) replacing brute-force graph operations with sampled or indexed approximations; (2) re-running the full cross-domain experiment with the latest architectural fixes to confirm probe-level improvements hold at scale; (3) integrating concept attention with modern state-space or transformer components; (4) testing on naturalistic analogy, causal reasoning, and continual-learning datasets; (5) separating cognitive plausibility claims from engineering performance claims; and (6) formalizing governance variables as measurable computational constructs.

\section{Conclusion}
RAVANA is a prototype pressure-driven cognitive architecture designed to study continual learning, analogy, memory consolidation, and self-regulation. Its central hypothesis is that cognition can be modeled as regulated interaction among prediction error, graph structure, replay, uncertainty, novelty, dissonance, and identity stability. Internal experiments show promising improvements in within-domain learning, cross-domain transfer (with subsequent architectural fixes achieving 100\% probe-level resolution), catastrophic-forgetting control, and synthetic fairness dynamics, but the evidence remains preliminary. For cognitive science, the value of the architecture is its explicit and testable integration of mechanisms that are often studied separately: local predictive error, structured concepts, sleep replay, global workspace broadcasting, and homeostatic governance.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Commit at time of submission:} \texttt{4880fd9}
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Cross-domain:} 14.3\% top-1, 71.4\% top-10 transfer accuracy (pre-fix); 100\% top-1 on probe set after architectural fixes (commit \texttt{4880fd9}).
