...
def test_stateful_equivalence():
    ...
    for tid in token_ids[:-1]:
        _, h_step = model.forward_step(tid, h_step, persist_activation=False, k_active_acf=7,
                                       fatigue_accumulation_rate=0.0, fatigue_decay_rate=0.0)
        
    context_full = [model._nearest_concept(model.token_embed(StateTensor(np.array([t]))).data[0]) for t in token_ids]
    logits_step, h_step = model.forward_step(token_ids[-1], h_step, persist_activation=False, k_active_acf=7,
                                             context_concepts=context_full,
                                             fatigue_accumulation_rate=0.0, fatigue_decay_rate=0.0)
    
    # Check hidden state equivalence
    h_diff = np.max(np.abs(h_seq - h_step))
    logits_diff = np.max(np.abs(logits_seq.data - logits_step.data))
    ...
    assert logits_diff < 1e-4, "Stateful recurrence logits mismatch!"
