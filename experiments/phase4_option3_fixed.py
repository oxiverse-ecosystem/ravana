"""
Phase 4 — Option 3 (Fixed): Conservative 32-dim baseline triplet-margin trajectory with corrected hardest negative loss.

This script runs the triplet-margin training loop with:
- 32-dim latent / 48-dim hidden / margin=0.05
- Corrected hardest-negative triplet loss (uses min/argmin distance instead of max/argmax)
- Original unaugmented TRAIN_TEXTS
- Logs trajectory to trajectory_option3_fixed.json
"""
...
# ──────────────────────────────────────────────────────────────────────────
# Training triples (in-domain words only) - ORIGINAL
# ──────────────────────────────────────────────────────────────────────────
...
def apply_triplet_margin(
    ...
    # Correct hardest negative: closest in distance space (minimum squared Euclidean distance)
    d_neg = np.min([d2(lat_a, lh) for lh in lats_h])
    ...
    if violation:
        # backprop toward positive, away from closest (hardest) negative
        hardest = lats_h[int(np.argmin([d2(lat_a, lh) for lh in lats_h]))]
...
def run(
    ...
    model_path = os.path.join(SCRIPT_DIR, "experiment_results", "encoder_32d_fixed.pkl")
