"""
Held-Out Transfer — Final Fix
==============================
Fixes:
1. Depth-2 results don't inherit parent confidence (decay by depth)
2. Return top-15 instead of top-8
...
def main():
    print("=" * 70)
    print("HELD-OUT TRANSFER — DEPTH DECAY + WIDER WINDOW")
    print("=" * 70)
    ...
    # FIX: depth decay — deeper results get lower confidence
    DEPTH_DECAY = 0.7  # each depth level multiplies by 0.7
    ...
    def traverse_single(subject, max_depth=3):
        ...
        for depth in range(1, max_depth + 1):
            next_frontier = set()
            depth_factor = DEPTH_DECAY ** (depth - 1)  # d1=1.0, d2=0.7, d3=0.49
            for nid in frontier:
    ...
    # ============================================================
    print("\n" + "=" * 70)
    print("TRANSFER TEST: depth decay + wider window")
    print("=" * 70)
    ...
    print(f"  MiniLM + depth decay + wider:  {bridge_correct/len(results):.0%} bridge, {queries_with_hits/total_queries:.0%} query, {total_hits/total_expected:.0%} object  <- NEW")
