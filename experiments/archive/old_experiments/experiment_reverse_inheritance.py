"""
Held-Out Transfer — Reverse Edge Inheritance
==============================================
Fix: when a bridge candidate has no outgoing edges for the query predicate,
traverse REVERSE edges to find children and inherit their properties.

Logic:
  fern → plant (bridge)
  plant has no outgoing "needs" edges
  But grass is_a plant (reverse edge)
  So plant inherits grass's outgoing edges
...
def main():
    print("=" * 70)
    print("HELD-OUT TRANSFER — REVERSE EDGE INHERITANCE")
    print("=" * 70)
    ...
    def traverse_single(subject, max_depth=3):
        """Traverse from subject with reverse edge inheritance."""
        ...
        # Find all nodes that have edges pointing TO this concept
        # e.g., if grass is_a plant, then plant inherits grass's properties
        reverse_parents = set()
        for eid, edge in model.graph.edges.items():
            if edge.target == subj_cid:
                # Check if this is an is_a edge
    ...
    def composed_query_fixed(novel_word, relation, top_k=15, bridge_k=5):
        ...
        for neighbor_word, sim in neighbors:
            ...
            # is a valid answer (fern→plant means plant is_a candidate)
    ...
    # ============================================================
    print("\n" + "=" * 70)
    print("TRANSFER TEST: reverse edge inheritance")
    print("=" * 70)
    ...
    for r in results:
        ...
        bridge = "B" if r["bridge_hits"] else "b"
        query = "Q" if term_hits > 0 else "q"
        print(f"    [{bridge}{query}] {r['term']:20s} objects={term_hits}/{term_expected}")

    print(f"\n  {'='*50}")
    print(f"  PROGRESS TRACKER:")
    print(f"  {'='*50}")
    print(f"  MiniLM + random projection:    42% bridge, 45% query, 45% object")
    print(f"  MiniLM + full-dim bridge:      67% bridge, 59% query, 52% object")
    print(f"  MiniLM + fixed multi-bridge:   67% bridge, 64% query, 61% object")
    print(f"  MiniLM + depth decay + wider:  67% bridge, 68% query, 68% object")
    print(f"  MiniLM + reverse inheritance:  {bridge_correct/len(results):.0%} bridge, {queries_with_hits/total_queries:.0%} query, {total_hits/total_expected:.0%} object  <- NEW")
