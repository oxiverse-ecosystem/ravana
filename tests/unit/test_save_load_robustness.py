...
def _fresh_engine(tag):
    e = CognitiveChatEngine(dim=64, seed=42, baby_mode=True,
                             data_dir=f"/tmp/ravana_m5_{tag}")
    return e


def test_schema_stamp_and_checksum_file():
    e = _fresh_engine("stamp")
    e.process_turn("hi")
    msg = e.save()
...
def test_partial_restore_on_corrupt_graph():
    ...
    with open(e._save_path, "wb") as f:
        pickle.dump(state, f)
    # Reload into a NEW engine — should partially restore, not blank.
    e2 = _fresh_engine("corrupt2")
    ok = e2.load()
