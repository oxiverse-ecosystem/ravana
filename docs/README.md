...
| [CAPABILITY_SELF_CUED_EPISODIC_RECALL.md](CAPABILITY_SELF_CUED_EPISODIC_RECALL.md) | How a follow-up question about content the user ALREADY disclosed ("where does meera restore clocks" after "my cousin meera restores antique clocks in pune") is answered from RAVANA's own record with no recall verb — retrieval-by-cue, running before the agentic hands and the internal knowledge base — with a structural majority-coverage + rarity bar that fails closed on one-shared-word and partial-overlap world questions, verified against the live engine. |
...
|[CAPABILITY_AFFECT_VERB_ATTITUDE_MINING.md](CAPABILITY_AFFECT_VERB_ATTITUDE_MINING.md) | How "lab-grown meat creeps me out" / "that flickering light freaks me out" / "his constant humming gets to me" are mined as a NEGATIVE stance on the subject (the old miner silently dropped them). Grammatical pattern: the affect verb is validated against the shared VAD affect lexicon (grown online via Hebbian learning), fail-closed on unknown verbs (0.0, skipped, no confabulation), every seen verb registered back into the matrix so coverage compounds; the stance lands in the same store a later "i changed my mind about X" recodes. No LLM, no second affect list, no per-topic table, verified against the live engine (6-test suite, green). |
|[CAPABILITY_DISTINCT_TOPIC_RECALL.md](CAPABILITY_DISTINCT_TOPIC_RECALL.md) | How "what did i tell you about sourdough?" returns ONLY the asked episode and never leaks sibling disclosures (the D7/D9 wrong/unrelated-turn echo). A structural detector (`_is_distinct_topic_recall`) cedes a single-topic recollection to the precise scoped retriever over the user's own disclosure transcript; fail-closed to the generic summary otherwise. No Q→A dict, no retraining, verified against the live engine (5-test suite, green; 112-pass recall/confab/memory regression, green). |
|[CAPABILITY_RESIDUAL_CLAUSE_RELATIONSHIP_MINING.md](CAPABILITY_RESIDUAL_CLAUSE_RELATIONSHIP_MINING.md) | How "my mentor taught me how to read the hive's mood" — a relationship disclosure whose verb OBJECT is a NON-NOUN-PHRASE CLAUSE — is now mined + recalled instead of dropped (the residual DEFECT B-variant). When the opinion-topic resolver yields nothing, the user's own clause words are kept verbatim (`user_model.py:2257`), bounded (`≤ 12` tokens) and framers stripped; noun-phrase objects still resolve to a concept head (regression-honored); pure verb-residue ("taught me") is honestly not stored. No LLM, no per-relationship table, no retraining, verified against the live engine (3-test suite, green). |
||[CAPABILITY_ACTIVITY_OBJECT_CATEGORY_RECALL_BRIDGE.md](CAPABILITY_ACTIVITY_OBJECT_CATEGORY_RECALL_BRIDGE.md)
||[CAPABILITY_DEGENERATE_HEAD_SKIP.md](CAPABILITY_DEGENERATE_HEAD_SKIP.md) | How the opinion-topic resolver skips a stop-word boundary when the head collected so far is entirely non-content (e.g. "night" in "out at night just to watch the stars"), instead of breaking and silently dropping the disclosure. Single structural conditional reading _OBJ_NONCONTENT, no authored reply, no per-topic rule, no retraining, verified against the live engine (5-test suite, green). |
||[CAPABILITY_CLAUSE_STRUCTURE_TOPIC_DETECTION.md](CAPABILITY_CLAUSE_STRUCTURE_TOPIC_DETECTION.md) | How opinion queries shaped as clauses ("do you think silence is underrated", "what do you think happens to a memory") extract the **real topic** (silence / memory) instead of leaking the predicate or verb into the stance key; bare trailing adverbs ("exactly") fall through to honest uncertainty instead of fabricating a stance. Clause detector (copula / intransitive-PP / bare-adverb vocabulary) before the flat extractor, no per-topic table, no LLM, verified against the live engine (4-test suite, green). |
 | How a category query ("what instrument do i play") bridges to a stored does:VERB activity ("i learn cello") even when the query verb != stored verb and the category word is absent from the value — seed `_ACTIVITY_ROLES` lexicon + runtime-growable `UserModel._activity_roles`, word-boundary match, live-fact reply, fail-closed, no LLM, no authored reply, verified against the live engine (6-test suite, green). |
