...
|[CAPABILITY_SINGLE_TOKEN_SELF_RECALL.md](CAPABILITY_SINGLE_TOKEN_SELF_RECALL.md) | How a self-recall query with only ONE content token (incl. "remind me") returns the stored reply instead of honest uncertainty — the recall gate's stopword set now includes "remind"/"reminded" and the minimum overlap drops to 1 for single-token queries (multi-token still >= 2, fail-closed), verified against the live engine (3-test suite, green). |
...
|[CAPABILITY_ACTIVITY_OBJECT_CATEGORY_RECALL_BRIDGE.md](CAPABILITY_ACTIVITY_OBJECT_CATEGORY_RECALL_BRIDGE.md) | How a category query ("what instrument do i play") bridges to a stored does:VERB activity ("i learn cello") even when the query verb != stored verb and the category word is absent from the value — seed `_ACTIVITY_ROLES` lexicon + runtime-growable `UserModel._activity_roles`, word-boundary match, live-fact reply, fail-closed, no LLM, no authored reply, verified against the live engine (6-test suite, green). |
