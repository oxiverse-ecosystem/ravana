...
class RLMv2(Module):
    ...
    def _rp_forward(self, subject_tid, rel_type_idx, route_softly=None):
        """Relation predictor forward pass. Direct latent -> domain logits (bypasses bottleneck)."""
        subject_tid = int(subject_tid)
        rel_type_idx = int(rel_type_idx)

        # Get robust source embedding (BY-PASS ENCODER: use embed directly)
    ...
    def forward(self, token_ids: np.ndarray) -> 'tensor':
        ...
        if ood_norm > 0:
            ...
            if np.any(valid_tok):
                ...
                sim_weight = 30.0 if disable_spreading else 3.0
