...
- **Recalls a single-topic gist from a one-token query (incl. "remind me").** Asked *"remind me what you said about silence"* — where only ONE content token survives stopword removal — it now returns the stored reply instead of honest uncertainty. Before, the recall gate required `_best_overlap >= 2`, which a single token could never satisfy. The fix: add `"remind"/"reminded"` to the stopword set (`engine.py:5819-5820`) and lower the threshold to 1 when `len(_q_set) <= 1` (`engine.py:5859`). Multi-token queries still require `>= 2` overlap to prevent confabulation. No LLM, no per-topic reply table. See `docs/CAPABILITY_SINGLE_TOKEN_SELF_RECALL.md`.
