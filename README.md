...
- **Recalls its own prior statement from a single content word (single-token recall).** Asked *"remind me what you said about privacy earlier"* — where only one content word ("privacy") survives stopword removal — it returns its actual stored reply about that topic instead of a canned greeting. The overlap threshold is adaptive (`_min_overlap = 1` when only one content token remains, else `>= 2`), and "remind" was added to the stopword set so it doesn't pollute the query candidates. Synonym queries also match via a GloVe-cosine fallback (`>= 0.7`) in `_match_fact` — *"afraid"* links to *"terrified"* — reusing the engine's seed embeddings (no new model, no retraining). No LLM, no per-topic reply table. See `docs/CAPABILITY_SINGLE_TOKEN_RECALL.md`.
