...
class SelfQueryMixin:
    ...
    def _route_self_query(self, user_input: str) -> Optional[str]:
        ...
        _self_introspect = re.search(
            r"\b(your|you)\b.*\b(read|line|take|view|mind|self|who you are|"
            r"what you (?:are|were)|how you (?:see|feel|think))\b", t)
        # Exclude if it's an opinion question with a concrete topic object
        # ("what do you think about X") — those belong to _agent_opinion.
        if _self_introspect and re.search(
                r"\b(think|thought|thinking|opinion|stance)\b.*\babout\s+\w", t):
