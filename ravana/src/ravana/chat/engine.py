            ...
            for _k, _f in pf.facts.items():
                ...
                if _k[1] == "does" and not getattr(_f, "superseded", False):
                    ...
                    if _wn in _v or (_wverb and _wverb == _fverb):
                        return f"you {_v}."
        _COUNT = re.search(
            r"\bhow\s+many\s+([a-z][a-z]+)(?:\s+[a-z][a-z]+)?\s+(do\s+i|did\s+i|have|keep|raise|"
            r"own|got|breed|run|keep on|have on|make|made|fire|fired|throw|threw|"
            r"build|built|brew|brewed|grow|grew|paint|painted|write|wrote)\b", q)
            ...
            for _k, _f in pf.facts.items():
                ...
                if _k[1] == "does":
                    ...
                    _m = re.match(r"^(?:keep|have|keep on|have on|raise|own|"
                                  r"breed|run|got|make|made|fire|fired|throw|"
                                  r"threw|build|built|brew|brewed|grow|grew|"
                                  r"paint|painted|write|wrote)\s+((?:one|two|"
                                  r"three|four|five|six|seven|eight|nine|ten|"
                                  r"eleven|twelve)|\d+)\b\s+", _v)
                    if _m and _cn in _v:
                        _best = _m.group(1)
                        break
